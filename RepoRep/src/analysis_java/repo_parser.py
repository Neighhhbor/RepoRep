...
class RepoParser:
    ...
    def generate_ast(self, file_path, file_node_id):
        ...
        def traverse(cursor, parent_id=None):
            ...
            if parent_id is not None:
                ...
                # self.project_graph.nodes[parent_id].setdefault('children', []).append({
                #     'id': node_id,
                #     'field_name': cursor.field_name  # 保存子节点的字段名称
                # })
    ...
    def add_edge(self, source, target, relationship, edge_type):
        edge_type_int = EDGE_TYPE_MAP.get(edge_type, 0)  # 使用EDGE_TYPE_MAP来映射边的类型到整数
...
def main():
    ...
    # repo_path = '/home/sxj/Desktop/Workspace/Development/RepoRepresentation/RepoRep/examples/Java/MyFirstClass'
    ...
    # 设置结果保存目录
    results_dir = os.path.join(os.path.dirname(__file__), 'output', repo_path.split('/')[-1])
