...
def display_community_info(community_id, partition, ig_G):
    """显示社区中的所有 AST 节点及其代码内容"""
    # 获取属于指定社区的所有节点 ID
    community_nodes = [i for i, membership in enumerate(partition.membership) if membership == community_id]
    st.write(f"### Community {community_id} Nodes and Attributes")

    for node_id in community_nodes:
        node = ig_G.vs[node_id]  # 获取节点对象
        node_name = node['name'] if 'name' in node.attributes() else f"node_{node_id}"
        st.write(f"- **Node Name**: {node_name}")

        # 只处理 AST 节点结构
        if 'file_id' in node.attributes():
            file_id = node['file_id']  # 获取 file_id
            try:
                # 查找文件节点并获取路径
                file_node = ig_G.vs.find(id=file_id)
                file_path = file_node['path']  # 获取文件路径
                ...
                with open(file_path, 'rb') as infile:
                    code = infile.read()[start_byte:end_byte].decode('utf-8')

                # 使用 Streamlit Expander 展开代码片段
            ...
            st.write("No associated file information.")
